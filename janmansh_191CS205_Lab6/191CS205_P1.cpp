...
string infix;  // infix expression (can be integers as well as alphabets without spaces)
...
void infixToPrefix(){
    ...
    while(1){
        ...
        if((next[0] >= 48 && next[0] <= 57)||(next[0] >= 'a' && next[0] <= 'z')||(next[0] >= 'A' && next[0] <= 'Z')){
            ...
            // popping the operator with precedence strictly greater than current
            else if(next[0] == '-'){
    ...
    cout<<"The infix to prefix expression is: "<<answer<<"\n";
}
